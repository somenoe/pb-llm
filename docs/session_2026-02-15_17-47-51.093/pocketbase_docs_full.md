# POCKETBASE DOCS|2026-02-15|53 sections
...
To get started:
-Install Go 1.21+
-Create a new project directory with main.go file inside it. This is what an example PocketBase
...
-To build a statically linked executable, you can run CGO_ENABLED=0 go build and then
start the created executable with ./myapp serve.
PocketBase embeds SQLite, but doesn&#39;t require CGO.
If CGO is enabled (aka. CGO_ENABLED=1), it will use
mattn/go-sqlite3
...
Returning an error or hook.StopPropagation in the handler function stops the hook execution
chain.
-Add(handler)
...
Pre/Add registration and pass it to Remove(id) or remove all handlers with
RemoveAll() (including system handlers).
...
onModel* event hooks.
It also exposes app.Dao().DB() builder that allows executing various SQL statements
information in the
dbx package godoc
### Executing queries
...
daos.New(db)/daos.NewMultiDB(concurrentDB, nonconcurrentDB):
...
## 24.Extend with Go - Record operations
...
## 25.Extend with Go - Collection operations
...
once.
And as a bonus, being .go files also ensures that the migrations will be embedded seamlessly in
your final executable.
### Enable the migrate command
The prebuilt executable enables the migrate command by default, but when you are extending PocketBase
...
// (use IsProbablyGoRun to enable it only during development, aka. when running with "go run")
Automigrate: osutils.IsProbablyGoRun(),
if err := app.Start(); err != nil {
log.Fatal(err)
The above example also shows the Automigrate config option which when enabled will create automatically
...
// add down queries...
return nil
The above will create a new blank migration file inside the configured command migrations directory.
...
## 27.Extend with Go - Jobs scheduling
...
## 28.Extend with Go - Console commands
...
## 29.Extend with Go - Sending emails
...
## 30.Extend with Go - Rendering templates
Response 200:
...
## 33.Extend with Go - Custom models
# Extend with Go - Custom models
### Custom model struct
The available models.Record and its helpers
...
models.Model
interface.
To make it a little bit easier, PocketBase also comes with a models.BaseModel struct that you
can embed in your model, leaving only the TableName() method to be implemented (aka. your collection
name). For example:
// article.go
package main
import (
...
works only with the
models.Record and you&#39;ll have to manually convert the models.Record to your
...
Discussion#2697).
To query your custom &quot;Article&quot; model you can use the app.Dao() instance and its query builder
(dbx).
##### Fetch articles
// article_query.go
package main
...
## 34.Extend with JavaScript - Overview
...
## 35.Extend with JavaScript - Event hooks
...
## 36.Extend with JavaScript - Routing
GET /hello/:name
...
## 37.Extend with JavaScript - Database
...
## 38.Extend with JavaScript - Record operations
...
## 39.Extend with JavaScript - Collection operations
...
## 40.Extend with JavaScript - Migrations
...
## 41.Extend with JavaScript - Jobs scheduling
...
## 42.Extend with JavaScript - Console commands
...
## 43.Extend with JavaScript - Sending emails
...
## 44.Extend with JavaScript - Sending HTTP requests
...
## 45.Extend with JavaScript - Rendering templates
Response 200:
{{template "placeholderName" .}}
Response 200:
{{block "placeholderName" .}}default...{{end}}
...
## 46.Extend with JavaScript - Logging
...
## 47.pb-ext - Enhanced PocketBase Server
# pb-ext
...
## 48.pb-ext - Scripts Documentation
## [COMMAND SEQUENCES]
...
## 49.pb-ext - Collections Implementation
```go
package main
// Collection example
import (
...
## 50.pb-ext - Handlers Implementation
```go
package main
// API_SOURCE
import (
...
## 52.pb-ext - Routes Implementation
```go
package main
// API_SOURCE
import (
...
## 53.pb-deployer - PocketBase Production Deployment
<div align="center">
